...
def getData(test=False):
  ...
  if clean_data:
    ...
    loquantiledf = pd.DataFrame(groups.apply(
        lambda x: x[features].quantile(0.2))).reset_index()
    ...
    for feature in features:
      train_df.loc[train_df[feature] < train_df[feature+'_loq'],
                   feature] = train_df[feature+'_loq']
      validation_df.loc[validation_df[feature] < validation_df[feature +
                                                               '_loq'], feature] = validation_df[feature+'_loq']
      test_df.loc[test_df[feature] < test_df[feature+'_loq'],
                  feature] = test_df[feature+'_loq']

  # normalize
  scaler = StandardScaler(with_mean=True, with_std=True, copy=True)
  scaler.fit(train_df.loc[:, features])   # get mean and std
  train_data = scaler.transform(
    train_df.loc[:, features].values)  # apply whitening
  train_labels = np.expand_dims(
      train_df.loc[:, 'truth'].values, axis=1).astype(np.int32)

  validation_data = scaler.transform(
      validation_df.loc[:, features].values)  # apply whitening
  validation_labels = np.expand_dims(
      validation_df.loc[:, 'truth'].values, axis=1).astype(np.int32)

  # normalize the test data as well
  test_data = scaler.transform(test_df.loc[:, features].values)
